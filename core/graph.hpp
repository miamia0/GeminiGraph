...
public:
  ...
  //边个数
  EdgeId edges;
  //out_degree out_degree[v]v的出边度数
  // 每个节点共有的信息
  // VertexId [vertices]; numa-aware
  VertexId *out_degree;
  //out_degree out_degree[v]v的入边度数
  ...
  // outgoing_edges 某个socket的出边总数
  // EdgeId [sockets]
  EdgeId *outgoing_edges;
  //
  ...
  //outgoing_adj_bitmap
  //sockets级别信息
  //存储本socket出边的bitmap信息
  //和outgoing_adj_index差不多，不过节约空间，可以快速判定
  //EdgeId [sockets] [vertices+1];
  Bitmap **outgoing_adj_bitmap;
  // EdgeId [sockets] [vertices+1]; numa-aware
  //某个socket中某个点的出边个数
  //之后更改为前缀和即compressed_outgoing_adj_index[s_i][p_v_i].index
  EdgeId **outgoing_adj_index;
  ...
  CompressedAdjIndexUnit **compressed_incoming_adj_index; // CompressedAdjIndexUnit [sockets] [...+1]; numa-aware
                                                          //compressed_outgoing_adj_vertices（某个socket的有出边的顶点总数）
  VertexId *compressed_outgoing_adj_vertices;
  //     compressed_outgoing_adj_index[s_i][v_i] 在第s_i个socket中存在的第v_i个节点的信息
  //     index信息如下
  //     edges|--------|-----|------|-----
  //           v1      v2    v5     v7
  //           index1  index2 index5 index7
...
// printf("interleave on %s\n", nodestring);
    ...
    MPI_Comm_rank(MPI_COMM_WORLD, &partition_id); //获得partitionid
    MPI_Comm_size(MPI_COMM_WORLD, &partitions);   //获得partition个数
    ...
    MPI_Barrier(MPI_COMM_WORLD); //mpi锁，起到同步作用
  }
...
#pragma omp parallel for
  ...
  void load_undirected_from_directed(std::string path, VertexId vertices)
  {
    ...
    //read_edge_buffer 文件读取每次读取的EdgeUnit buffer
    ...
    while (read_bytes < bytes_to_read)
    {
      ...
      //剩余数据大小大于edge_unit_size * CHUNKSIZE
    ...
    /* 计算
    partition_offset 对图进行节点上的分块
    每一个节点k管理的顶点是在partitions[k]与partitions[k+1]区间上
    */
    ...
    for (int i = 0; i < partitions; i++)
    {
      ...
      //最后一个partition终结位置是vertices
      if (remained_partitions == 1)
      {
        partition_offset[i + 1] = vertices;
      }
      else //如果不是最后一个节点需要按照混合度量方法进行分配节点
      {
        //got_edges已经取到的边数
        EdgeId got_edges = 0;
    ...
    */
    ...
    /*
    将out_degree变更为numa-aware的数组。只可内部访问
    这个数组，由我（numa）来接管啦。
    */
    //申请大小为vertices的numa-aware数组
    ...
    */
    ...
    }
    {
      std::thread recv_thread_dst([&]() {
        ...
        {
          //MPI_Probe函数探测接收消息的内容，但不影响实际接收到的消息。
          //我们可以根据探测到的消息内容决定如何接收这些消息
          ...
          int recv_bytes;
          //PI_Probe可用于查找消息的大小,MPI_Get_count来获取该大小
          ...
          /*对于收到的边
          更新到outgoing_adj_bitmap与outgoing_adj_index上
          */
          for (EdgeId e_i = 0; e_i < recv_edges; e_i++)
          {
            ...
            if (!outgoing_adj_bitmap[dst_part]->get_bit(src))
            {
              ...
              outgoing_adj_index[dst_part][src] = 0; ////初始化为0
            }
    ...
    outgoing_edges 某个socket的出边总数（边数）
    更新outgoing_adj_index为了下一步计算outgoing_adj_list
    其实这几百行代码就是为了完成outgoing_adj_list中的基数排序
    */
    ...
    for (int s_i = 0; s_i < sockets; s_i++)
    {
      ...
      compressed_outgoing_adj_vertices[s_i] = 0;
      /*
      compressed_outgoing_adj_vertices用于统计当前已经计算的顶点个数，最终是socket中出边的个数
      compressed_outgoing_adj_index[s_i][v_i] 在第s_i个socket中存在的第v_i个节点的信息
      index信息如下
      edges|--------|-----|------|-----
            v1      v2    v5     v7   
